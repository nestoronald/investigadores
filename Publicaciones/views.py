...
def investigador(request,idinv=1):
    ...
    db.close()
    ctx = {'investigador':Investigador.objects.get(id=idinv)}
...
def publication_list(request):
    ...
    cursor.execute("""SELECT  * FROM data d, subcategory s, category c
        WHERE d.idsubcategory = s.idsubcategory AND s.idcategory = c.idcategory AND s.subcategory_enable =1 and d.iddata=1
        AND (ExtractValue( data_content,  "publicaciones/authorPRI/idauthor0"  )  =  "569") LIMIT 0 , 30""")
